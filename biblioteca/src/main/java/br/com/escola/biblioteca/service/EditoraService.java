...
public class EditoraService {
	...
	public List<EditoraResponseDTO> listar() {
		return editoraRepository.findAll().stream().map(EditoraResponseDTO::fromEntity).collect(Collectors.toList());
	}

	public EditoraResponseDTO buscarPorId(Long id) {
		return editoraRepository.findById(id).map(EditoraResponseDTO::fromEntity)
	...
	public EditoraResponseDTO atualizar(Long id, EditoraRequestDTO dto) {
		...
		editoraRepository.findAll().stream().filter(ex -> !ex.getId().equals(id)).forEach(ex -> {
